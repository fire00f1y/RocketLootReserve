...
local queueMeta = {
    __index = function(_, k)
        assert(Queue[k], "Queue has no field: " .. tostring(k) .. ". Make sure ItemCache is up to date.")
        return Queue[k]
    end,
...
local retrieveModes = {
    cache = { Retrieve = function(self)
        return self:Cache()
    end, IsRetrieved = function(self)
        return self:IsCached()
    end },
    load = { Retrieve = function(self)
        return self:Load()
    end, IsRetrieved = function(self)
...
local callbackControllerMeta = {
    __index = function(_, k)
        assert(CallbackController[k], "CallbackController has no field: " .. tostring(k) .. ". Make sure ItemCache is up to date.")
...
local itemMeta = {
    __index = function(_, k)
        assert(Item[k], "Item has no field: " .. tostring(k) .. ". Make sure ItemCache is up to date.")
        return Item[k]
    end,
    ...
    __eq = function(item1, item2)
        return item1:GetID() == item2:GetID() and item1:GetSuffix() == item2:GetSuffix()
    end,
    __lt = function(item1, item2)
        return (item1:GetName() or "") < (item2:GetName() or "")
    end,
    __le = function(item1, item2)
        return (item1:GetName() or "") <= (item2:GetName() or "")
    end,
    __tostring = function(self)
...
function ItemDB:Store(item)
    ...
    elseif item:GetSuffix() and not storage[item:GetID()][item:GetSuffix()] then
        -- only store suffix items if the suffix matters
...
function ItemDB:InitMouseoverHook()
    ...
    GameTooltip:HookScript("OnTooltipSetItem", function(tooltip)
        if not self.mouseoverHook then
            return
        end
...
function ItemDB:InitGetItemInfoHook()
    ...
    hooksecurefunc("GetItemInfo", function(...)
        if not self.GetItemInfoHook then
            return
        end
...
function Item:RequiresSkill(skill, level)
    ...
    if skillRequired == nil then
        return nil
    end
    return (not skill or skill == skillRequired) and (not level or level <= levelRequired) and true or false
end

function Item:RequiresCooking(...)
    return self:RequiresSkill(PROFESSIONS_COOKING, ...)
end
function Item:RequiresFirstAid(...)
    return self:RequiresSkill(PROFESSIONS_FIRST_AID, ...)
end
function Item:RequiresFishing(...)
    return self:RequiresSkill(PROFESSIONS_FISHING, ...)
end
...
function Item:DoesNotBind()
    return self:GetBindType() == LE_ITEM_BIND_NONE
end
function Item:CanBind()
    return self:GetBindType() ~= LE_ITEM_BIND_NONE
end
function Item:IsBindOnPickup()
    return self:GetBindType() == LE_ITEM_BIND_ON_ACQUIRE
end
function Item:IsBindOnEquip()
    return self:GetBindType() == LE_ITEM_BIND_ON_EQUIP
end
function Item:IsBindOnUse()
    return self:GetBindType() == LE_ITEM_BIND_ON_USE
end
...
function Item:IsEquippable()
    return self:GetEquipLocation() ~= "" and self:GetEquipLocation() ~= "INVTYPE_NON_EQUIP"
end
function Item:IsHelm()
    return self:GetEquipLocation() == "INVTYPE_HEAD"
end
function Item:IsNecklace()
    return self:GetEquipLocation() == "INVTYPE_NECK"
end
function Item:IsShoulder()
    return self:GetEquipLocation() == "INVTYPE_SHOULDER"
end
function Item:IsShirt()
    return self:GetEquipLocation() == "INVTYPE_BODY"
end
function Item:IsTabard()
    return self:GetEquipLocation() == "INVTYPE_TABARD"
end
function Item:IsChest()
    return self:GetEquipLocation() == "INVTYPE_CHEST" or self:GetEquipLocation() == "INVTYPE_ROBE"
end
function Item:IsBelt()
    return self:GetEquipLocation() == "INVTYPE_WAIST"
end
function Item:IsPants()
    return self:GetEquipLocation() == "INVTYPE_LEGS"
end
function Item:IsBoots()
    return self:GetEquipLocation() == "INVTYPE_FEET"
end
function Item:IsBracers()
    return self:GetEquipLocation() == "INVTYPE_WRIST"
end
function Item:IsGloves()
    return self:GetEquipLocation() == "INVTYPE_HAND"
end
function Item:IsRing()
    return self:GetEquipLocation() == "INVTYPE_FINGER"
end
function Item:IsTrinket()
    return self:GetEquipLocation() == "INVTYPE_TRINKET"
end
function Item:IsCloak()
    return self:GetEquipLocation() == "INVTYPE_CLOAK"
end

function Item:IsQuiver()
    return self:GetEquipLocation() == "INVTYPE_QUIVER"
end
function Item:IsNormalBag()
    return self:GetEquipLocation() == "INVTYPE_BAG"
end

function Item:IsRelic()
    return self:GetEquipLocation() == "INVTYPE_RELIC"
end
function Item:IsShield()
    return self:GetEquipLocation() == "INVTYPE_SHIELD"
end
function Item:IsHoldable()
    return self:GetEquipLocation() == "INVTYPE_HOLDABLE"
end

function Item:IsOneHandWeapon()
    return self:GetEquipLocation() == "INVTYPE_WEAPON"
end
function Item:IsTwoHandWeapon()
    return self:GetEquipLocation() == "INVTYPE_2HWEAPON"
end
function Item:IsMainHandWeapon()
    return self:GetEquipLocation() == "INVTYPE_WEAPONMAINHAND"
end
function Item:IsOffHandWeapon()
    return self:GetEquipLocation() == "INVTYPE_WEAPONOFFHAND"
end

function Item:IsGunOrBow()
    return self:GetEquipLocation() == "INVTYPE_RANGED"
end
function Item:IsThrownWeapon()
    return self:GetEquipLocation() == "INVTYPE_THROWN"
end

function Item:IsBag()
    return self:IsNormalBag() or self:IsQuiver()
end
function Item:IsOffHand()
    return self:IsRelic() or self:IsShield() or self:IsHoldable() or self:IsOffHandWeapon()
end
function Item:IsRangedWeapon()
    return self:IsGunOrBow() or self:IsThrownWeapon()
end
function Item:IsMeleeWeapon()
    return self:IsOneHandWeapon() or self:IsTwoHandWeapon() or self:IsMainHandWeapon() or self:IsOffHandWeapon()
end
